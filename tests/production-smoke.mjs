...
const EXPECTED_BUILD='20260919-r45';
const errors=[];
...
async function inspect(viewport,name){
  ...
  try{
    ...
    const headerFrame=page.frameLocator('#v4frame');
    check(await headerFrame.locator('header.top #menuBtn').count()===0,name+': redundant top menu/filter button returned');
    check(await headerFrame.locator('header.top #focusSearch').count()===1,name+': top search button missing');
